...
lemma two_pow_lt_two_pow (w : Nat) : 2^w < 2^(w + 1) :=
  pow_lt_pow_right (one_lt_succ_succ 0) (lt.base w)
...
lemma shiftRight_eq_testBit : Nat.shiftRight x i % 2 = (x.testBit i).toNat := by simp [Nat.testBit, Nat.mod_two_of_bodd]

lemma div_add_mod_two_pow (m n : Nat) : n = 2 ^ m * n >>> m + n % (2 ^ m) := by simp_rw [Nat.shiftRight_eq_div_pow,Nat.div_add_mod]

theorem mod_two_pow_succ (x i : Nat) : x % 2 ^ (i + 1) = 2 ^ i * (Nat.testBit x i).toNat + x % (2 ^ i):= by
  have h1 := div_add_mod_two_pow i x
  rw [← div_add_mod (x >>> i) 2,mul_add, ← mul_assoc, ← pow_succ] at h1
  have rr : (x >>> i % 2) = (x.testBit i).toNat := shiftRight_eq_testBit
  rw[rr] at h1 --   rw [shiftRight_eq_testBit] at h1
  have := lt_succ_two_pow (toNat_le_one (x.testBit i)) (mod_lt x (NeZero.pos (2^i)))
  simp [(Nat.div_mod_unique (Nat.two_pow_pos (i+1))).mpr ⟨add_rotate _ _ (x%(2^i)) ▸ h1.symm, this⟩]
...
theorem eq_of_testBit_eq_iff {n m : Nat} : (∀ i, n.testBit i = m.testBit i) = (n=m) := by
  ...
  · intro h; simp [eq_of_testBit_eq h]
  · intro h i; simp [h]
...
@[simp] def bitwise_mul (x y n: Nat) := toNat (λ j => bitwise_mul.res x y j (n - 1)) 0 n
...
theorem scale_two_pow_bit_new (h: y< 2^w): ((bitwise_mul.res x (2^w+y) w w) = ((bitwise_mul.res x y w w) ^^ (x.testBit 0))) ∧  (bitwise_mul.carry x (2^w+y) w w= bitwise_mul.carry x y w w) := by
  ...
  · cases' h3 : y.testBit 0
    . simp only [zero_eq, _root_.pow_zero, res.eq_1, sh, le_refl, tsub_eq_zero_of_le, testBit_add, h3,
          Bool.xor_false, bitwise_carry, and_true, Bool.decide_coe, ite_true, and_false, decide_False, ite_self,
          carry.eq_1, and_self]
      simp only [testBit_zero, decide_eq_true_eq, mod_succ, decide_True, and_true, Bool.false_bne]
    . simp only [ _root_.pow_zero, and_false, and_self, and_true, bitwise_carry, Bool.decide_coe,
        Bool.not_eq_true', Bool.xor_false, Bool.xor_self, Bool.xor_true, carry.eq_1, decide_False,
        h3, ite_self, ite_true, le_refl, res.eq_1, sh, testBit_add, tsub_eq_zero_of_le, zero_eq ]
      simp only [testBit_zero, decide_eq_true_eq, mod_succ, decide_True, Bool.true_eq_false,
        and_false, decide_False]
  · simp only [ add_le_iff_nonpos_right, and_false, and_self, and_true, Bool.and_false, Bool.decide_coe,
        Bool.false_or, Bool.xor_false, carry.eq_3, decide_False, ge_iff_le, ite_false, ite_self,
        ite_true, le_add_iff_nonneg_right, le_refl, lt_self_iff_false, lt_two_pow_testBit_false h (show w + 1 ≤ w + 1 by simp),
        res_scale_two_pow_new (Or.inr (lt_succ_self w)), res.eq_3, sh, testBit_translate_one' h, tsub_eq_zero_of_le]

theorem res_n_eq_res_w (h: w ≤ n ): bitwise_mul.res x y w n = bitwise_mul.res x y w w:= by
  have ⟨k, hk⟩ := Nat.exists_eq_add_of_le h; clear h
  cases' w with w
...
theorem carry_n_eq_false (h: w < n ) (h0: y < 2^(w+1) ): bitwise_mul.carry x y m n = false:= by
  ...
  · induction' m with m ih
    ...
    · simp only [carry.eq_3, sh, ge_iff_le, lt_two_pow_testBit_false h0 (show w + 1 ≤ succ n by linarith),
        and_false, decide_False, ite_self, Bool.and_false, Bool.xor_false, ih, Bool.or_self]
...
theorem res_n_eq_res_w' (h: w ≤ n) (h1: w ≤ i) (h0: y< 2^(w+1)): bitwise_mul.res x y i n = bitwise_mul.res x y i w := by
  ...
  · have ⟨k, hk1, hk2⟩ :∃ k, n = w+k ∧ 0 < k := by use n-w; rw[add_comm]; simp [Nat.sub_add_cancel (le_of_lt h2), h2]
    ...
    · induction' k with k ih generalizing n
      · simp at hk2
      ·
        simp only [hk1, add_succ, res.eq_3, sh, add_zero, ge_iff_le, succ_sub_succ_eq_sub, Bool.decide_and,
          Bool.decide_coe, carry.eq_3, Bool.xor_assoc]
        have H : testBit y (succ (w + k)) = false := lt_two_pow_testBit_false h0 (show w+1 ≤ succ (w+k) by linarith)
        simp only [ge_iff_le, H, Bool.and_false, ite_self, Bool.xor_false,
          carry_n_eq_false (show w < succ (w + k) by linarith) h0, Bool.or_self, Bool.xor_self]
        cases' k with k
        · simp only [succ_eq_add_one, add_zero]
...
theorem mul_two_pow (h: j < i): testBit (x*2^i) j = false := by
  ...
  have: x * 2 ^ i / 2 ^ j = 2 ^ (i-j)*x := by
    rw [Nat.div_eq_iff_eq_mul_right (Nat.two_pow_pos j) (dvd_mul_of_dvd_right (pow_dvd_pow 2 (le_of_lt h)) x), ← mul_assoc, ← pow_add]
...
theorem mul_two_pow_gen (h: i ≤ n) : testBit (x*2^i) n = testBit x (n-i) := by
  simp only [testBit, shiftRight_eq_div_pow]
  nth_rewrite 1 [← Nat.sub_add_cancel h, pow_add, Nat.mul_div_mul_right _ _ (Nat.two_pow_pos i)]; rfl


theorem helper_of_main (_h0: 0<i) (H2: j < 2*n) (h2: i ≤ n) (hij: j < i): bitwise_carry (x * 2 ^ i) (x * (y % 2 ^ i)) j = false := by
...
theorem helper_of_main1 (h0: 0<i) (H2: j < 2*n) (h2: i ≤ n) (h: ∀ k < 2*n, bitwise_mul.res x ( y % 2 ^ i) k n = testBit ( x * (y % 2 ^ i)) k ) (h1: i ≤ j): bitwise_carry (x * 2 ^ i) (x * (y % 2 ^ i)) j = bitwise_mul.carry x (2 ^ i + y % 2 ^ i) j i := by
  have H: (y % (2^i)) < 2^i :=by simp [Nat.two_pow_pos, mod_lt]
...
theorem bitwise_mul_eq_mul' (y: Nat)  (h: i ≤ n) (h1: x < 2^n): bitwise_mul x (y%(2^i)) (2*n) = x * (y%(2^i)):= by
  ...
  · simp [Hn] at h; simp [h, Hn, mod_one, res_zero]
  apply eq_of_testBit_eq_lt (toNat_lt) (by linarith [pow_add 2 i n, Nat.mul_lt_mul' (le_of_lt (mod_lt y (two_pow_pos i))) h1 (two_pow_pos i), (pow_le_pow (show 1 ≤ 2 by linarith) (show i+n ≤ 2 * n by linarith[h]))])
...
theorem testBit_eq_mod_two_pow (h: j < n): x.testBit j = (x%(2^n)).testBit j := by
  ...
  · have := (dvd_iff_mod_eq_zero (2 ^ j) (2^n * Bool.toNat (testBit x n))).1 (dvd_mul_of_dvd_left (pow_dvd_pow 2 (le_of_lt_succ h)) (Bool.toNat (testBit x n)))
    ...
    · have h2:= @mul_two_pow j n (Bool.toNat (testBit x n)) h1
      simp_rw [testBit, shiftRight_eq_div_pow, Nat.mul_comm, h2, ih h1] at *
      aesop
    · rw [h1]
      cases' h2: bodd (x/2^n)
      <;> simp [Nat.div_eq_zero, testBit, h2, shiftRight_eq_div_pow]
...
theorem bitwise_mul_eq_mul (h: 0 < n) : bitwise_mul x y n = (x*y)%2^n := by
  ...
  intros j hj
  rw [← testBit_eq_mod_two_pow hj, toNat_testBit hj, bitwise_mul, toNat_testBit (show j < 2*n by linarith)]
...
theorem testBit_mul : (x*y).testBit i = bitwise_mul.res x y i i := by
  ...
  rw [← mod_eq_of_lt hn2]
  rw [← bitwise_mul_eq_mul' y rfl.ge hn1, bitwise_mul, toNat_testBit (by linarith)]
...
theorem shiftRightight_eq_shiftRight: n >>> m = Nat.shiftRight n m := by
  simp only [HshiftRightight.hshiftRightight, shiftRightight.shiftRightight]
  induction' m with m hm generalizing n
  <;> simp [Nat.shiftRight, Nat.shiftRightight, Nat.div2_val, *]

theorem bitwise_extract_eq_extract : bitwise_extract x i j = (x >>> j)%(2^(i - j + 1)):= by
  apply eq_of_testBit_eq_lt (toNat_lt) (by simp [mod_lt, two_pow_pos])
  intros k hk
  rw [toNat_testBit hk, ← testBit_eq_mod_two_pow hk]
  simp [shiftRightight_eq_shiftRight, testBit, shiftRight_eq_div_pow, Nat.div_div_eq_div_mul, pow_add, mul_comm]
...
lemma lt_carry_false (h : 0 < b) (ha : a < 2^w) (hb : b < 2^w): a < b ↔ !(bitwise_carry a (bitwise_neg b w) w) := by
  ...
  · contrapose!; intro h2
    ...
    rw [testBit, shiftRight_eq_div_pow, Nat.add_div_of_dvd_right (dvd_refl _)]
    simp [Nat.div_eq_zero (lt_of_le_of_lt (sub_le _ _) ha)]

lemma shiftRightight_eq_div_two : a >>> 1 = a / 2 := shiftRightight_eq_shiftRight ▸ shiftRight_eq_div_pow a 1
...
lemma uDivModRec_one (h0 : 0 < b) (h : b < 2^w) : uDivModRec.go 1 b w (v + 1) = (1 / b, 1 % b) := by
  ...
  · simp only [uDivModRec.go, ite_false, bitwise_add_eq_add, shiftLeft_eq_two_mul, shiftRightight_eq_div_two]
...
lemma uDivModRec_ih (hb : b < 2^w ) (ha : a < 2^v) (h0 : 0 < b) (hv : v ≤ w): uDivModRec.go a b w v = (a / b, a % b) := by
  ...
  · induction' a using Nat.strongInductionOn with a ih generalizing v
    ...
    · simp only [uDivModRec.go, beq_iff_eq, (show a ≠ 0 by linarith), ite_false, Nat.add]
      ...
      simp only [shiftRightight_eq_div_two, shiftLeft_eq_two_mul, bitwise_add_eq_add, ih ((div_lt_iff_lt_mul _).mpr (pow_succ 2 v ▸ ha)) (le_trans (by linarith) hv)]
...
#check shiftRight_eq_testBit
lemma testBit_eq_shift (hx : x < 2^(w + 1)) : x >>> w = (x.testBit w).toNat := by
  rw [shiftRightight_eq_shiftRight, testBit, bodd_toNat]
...
theorem ge_of_testBit_eq_true (h: x < 2^(w + 1)) (h1 : x.testBit w = true) : 2^w ≤ x := by
  ...
  simp only [← testBit_eq_shift h, toNat_true, shiftRightight_eq_shiftRight, shiftRight_eq_div_pow] at h1
